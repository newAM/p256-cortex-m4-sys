...
extern "C" {
    fn P256_divsteps2_31(delta: i32, f: u32, g: u32, res_matrix: &[u32; 4]) -> i32;
    fn P256_matrix_mul_fg_9(a: u32, b: u32, fg: &[FGInteger; 2], res: &mut FGInteger);
    fn P256_matrix_mul_mod_n(a: u32, b: u32, xy: &[XYInteger; 2], res: &mut XYInteger);
    fn P256_to_montgomery(aR: &mut [u32; 8], a: &[u32; 8]);
    fn P256_from_montgomery(a: &mut [u32; 8], aR: &[u32; 8]);
    fn P256_check_range_p(&a: [u32; 8]) -> bool;
    fn P256_check_range_n(&a: [u32; 8]) -> bool;
    // fn P256_mul_mod_n(uint32_t res[8], const uint32_t a[8], const uint32_t b[8]);
    // fn P256_add_mod_n(uint32_t res[8], const uint32_t a[8], const uint32_t b[8]);
    // fn P256_mod_n_inv_vartime(uint32_t res[8], const uint32_t a[8]);
    // fn P256_reduce_mod_n_32bytes(uint32_t res[8], const uint32_t a[8]);
    // fn P256_select_point(uint32_t (*output)[8], uint32_t* table, uint32_t num_coordinates, uint32_t index);
    // fn P256_jacobian_to_affine(uint32_t affine_mont_x[8], uint32_t affine_mont_y[8], const uint32_t jacobian_mont[3][8]);
    // fn P256_point_is_on_curve(const uint32_t x_mont[8], const uint32_t y_mont[8]) -> bool;
    // fn P256_decompress_point(uint32_t y[8], const uint32_t x[8], uint32_t y_parity) -> bool;
    // fn P256_double_j(uint32_t jacobian_point_out[3][8], const uint32_t jacobian_point_in[3][8]);
    // fn P256_add_sub_j(uint32_t jacobian_point1[3][8], const uint32_t (*point2)[8], bool is_sub, bool p2_is_affine);
    // fn P256_verify_last_step(const uint32_t r[8], const uint32_t jacobian_point[3][8]) -> bool;
    // fn P256_negate_mod_p_if(uint32_t out[8], const uint32_t in[8], uint32_t should_negate);
    // fn P256_negate_mod_n_if(uint32_t out[8], const uint32_t in[8], uint32_t should_negate);
...
fn hash_to_z(z: &mut [u32; 8], hash: &[u8]) {
    let hash_len: usize = core::cmp::min(32, hash.len());

    let z_u8: &mut [u8; 32] = unsafe { core::mem::transmute::<&mut [u32; 8], &mut [u8; 32]>(z) };

    for i in 0..hash_len {
        z_u8[i] = hash[hash_len - 1 - i];
    }

    for i in hash_len..32 {
        z_u8[i] = 0;
    }
}
