...
impl SecretKey {
    ...
    pub fn random(rng: impl CryptoRng + RngCore) -> Self {
        ...
        loop {
            ...
            let valid = unsafe { P256_check_range_n(&secret.0[0] as *const u32) };
            if valid {
                return secret;
            }
        }
    ...
    pub fn from_bytes(bytes: impl AsRef<[u8]>) -> Option<Self> {
        ...
        if !unsafe { P256_check_range_n(&secret.0[0] as *const u32) } {
            return None;
        }
        Some(secret)
    }
    ...
    /// ECDH key agreement.
    pub fn agree(&self, other: &PublicKey) -> Option<SharedSecret> {
...
impl PublicKey {
    ...
    pub fn from_untagged_bytes(bytes: &[u8]) -> Option<Self> {
        if bytes.len() != 64 {
            return None;
        }
    ...
    pub fn to_compressed_sec1_bytes(&self) -> [u8; 33] {
        ...
        unsafe {
            p256_point_to_octet_string_compressed(
    ...
    pub fn to_uncompressed_sec1_bytes(&self) -> [u8; 65] {
        ...
        unsafe {
            p256_point_to_octet_string_uncompressed(
                bytes.as_mut_ptr() as *mut _,
                &self.x[0] as *const _,
                &self.y[0] as *const _,
            );
...
impl Signature {
    ...
    pub fn from_untagged_bytes(bytes: &[u8]) -> Option<Self> {
        ...
        let valid_r = unsafe { P256_check_range_n(&signature.r[0] as *const u32) };
        ...
        let valid_s = unsafe { P256_check_range_n(&signature.r[0] as *const u32) };
